...
parser.add_argument('--transfer', action='store_true', help='Transfer data from zinc to cluster')
...
def main():
        ...
        if args.run:
            ...
            for i, run in enumerate(args.run):
                if '_' in run:  # run name, yyyymmdd_hhmm
                    transfer_daemon.DoOneRun(run, i)
                else: # run number
                    for row in RunsDB.Select(selection='name',cuts='number==%s' % run):
                        transfer_daemon.DoOneRun(row[0],i)
        else:
    ...
    if args.process:
        if args.run:
            ...
            for i, run in enumerate(args.run):
                if '_' in run:  # run name, yyyymmdd_hhmm
                    process_daemon.DoOneRun(run, i)
                else: # run number
                    for row in RunsDB.Select(selection='name',cuts='number==%s' % run):
                        process_daemon.DoOneRun(row[0],i)
        else:
